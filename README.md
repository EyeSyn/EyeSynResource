...
*The source code of VerbalGaze model is available [**here**](https://github.com/EyeSyn/EyeSynResource/blob/main/verbalGazeModel/verbalGazeCodes.zip)*. Please refer to section 4.3 in the paper for details about the design. The VerbalGaze Model containts the following scripts: 
```
verbalGazeCodes
...
*The source code of DynamicScene model with example is available [**here**](https://github.com/EyeSyn/EyeSynResource/blob/main/scenePerceptionModel/dynamicScenePerceptionCodes.zip)*. Please refer to section 4.4 in the paper for details about the design. The DynamicScene Model containts the following scripts: 
```
dynamicScenePerceptionCodes
└───mainDynamicScene.m  ##The main script
└───mainVideoWeighted.m  ## Implementation of the dynamic scene perception. 
└───fixationPD.m  ## Probability distribution of the fixation duration.
└───gazeSythesisOnFixation.m  ## Synthesis gaze points on the selected fixation positions.
```
Note that you need to download the [SaliencyToolbox](https://www.saliencytoolbox.net/doc/index.html#installation) for the bottom-up saliency-based fixation estimation. In the paper, we extract 50 short documentary videos from the online video series of the “*National Geographic Animals 101*” [[8]](https://www.youtube.com/playlist?list=PLaP7riDmeeBVAZOy_l1jrvJpPENNZYqts) as the inputs of the **DynamicScene model**. Each video lasts for 2 to 6 minutes. 
